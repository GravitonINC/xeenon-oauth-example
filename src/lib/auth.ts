...
async function refreshAccessToken(token: JWTToken): Promise<JWTToken> {
  try {
    ...
    const response = await fetch(process.env.XEENON_TOKEN_ENDPOINT!, {
      method: 'POST',
      headers: {
  ...
  providers: [
    {
      ...
      checks: ['pkce', 'state'],
      token: process.env.XEENON_TOKEN_ENDPOINT!,
      clientId: process.env.XEENON_CLIENT_ID!,
      clientSecret: process.env.XEENON_CLIENT_SECRET!,
      userinfo: process.env.XEENON_USERINFO_ENDPOINT!,
